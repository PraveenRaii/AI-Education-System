...
function ProfileOverview({ user }) {
  ...
  return <section className="profile-overview">
    <div className="profile-hero panel"><div className="profile-avatar">{user?.name?.slice(0, 2).toUpperCase()}</div><div><span className="section-kicker">YOUR NEURAL ACADEMY PROFILE</span><h2>{user?.name}</h2><p>{user?.email}</p></div><span className="profile-role">{user?.role || 'student'}</span></div>
