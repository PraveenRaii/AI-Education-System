...
function SettingsView({ user, onUserUpdate, theme, setTheme }) {
  ...
  async function saveProfile(event) {
    ...
    try {
      ...
      onUserUpdate(data.user); localStorage.setItem('neural_academy_user', JSON.stringify(data.user)); setMessage('Profile updated successfully.');
  ...
  async function uploadAvatar(event) {
    ...
    try {
      ...
      const response = await fetch('/api/uploads/avatar', { method: 'POST', headers: { Authorization: `Bearer ${token}` }, body });
      const data = await response.json(); if (!response.ok) throw new Error(data.message || 'Upload failed.');
      const updated = { ...user, avatarUrl: data.url }; onUserUpdate(updated); localStorage.setItem('neural_academy_user', JSON.stringify(updated)); setMessage('Profile photo updated.');
